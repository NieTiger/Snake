...
void draw_border(game_t *gs) {
  ...
  rectangle(gs->border.y1, gs->border.x1, gs->border.y2, gs->border.x2);
...
void draw_ball(game_t *gs) {
  if (gs->ball
          .flag) { // set flag will calculate a new random position for the ball
    gs->ball.flag = 0;

    do {
      gs->ball.x = gs->border.x1 + 2 +
                   rand() % ((gs->border.x2 - gs->border.x1) / 2 - 1) * 2;
      gs->ball.y =
          gs->border.y1 + 1 + rand() % (gs->border.y2 - 1 - gs->border.y1);
...
void detect_collision(game_t *gs) {
  ...
  if (gs->ball.x == *snake_x && gs->ball.y == *snake_y) {
